...
contract EthereumDIDRegistry {
  ...
  function checkSignature(address identity, uint8 sigV, bytes32 sigR, bytes32 sigS, bytes32 hash) internal returns(address) {
    // console.log("checkSignature", identity);

    address signer = ecrecover(hash, sigV, sigR, sigS);

    // console.log("checkSignature", signer);
  ...
  function _bulkAddSigned(
    DelegateParamSigned[] calldata signedDelegateParams,
    AttributeParamSigned[] calldata signedAttributeParams
  ) internal {
  ...
  function bulkAdd(
    ...
    // signed transaction
    _bulkAddSigned(signedDelegateParams, signedAttributeParams);
  }
  ...
  function bulkRevoke(
    ...
    _bulkRevokeSigned(signedDelegateParams, signedAttributeParams);
  }
}
